...
using MwcModApi.Parts.Trigger;
using MwcModApi.Tools;
using UnityEngine;
using static MwcModApi.Parts.EventSystem.PartEvent;
...
namespace MwcModApi.Parts
{
	...
	public class Part : BasicPart, ISupportsPartEvents, SupportsPartBehaviourEvents
	{
		...
		)
		{
			...
			Setup(
				id, name, parent, installPosition, installRotation, partBaseInfo,
		...
		{
			Setup(
				id, name, null, Vector3.zero, Vector3.zero, partBaseInfo,
		...
		public Part(
			...
			id, name, (BasicPart) parent, installPosition, installRotation,
		...
		protected void Setup(
			...
			DisableCollision disableCollisionWhenInstalled,
			string prefabName
		)
		{
			this.id = id;
			...
			if (!MwcModApi.modSaveFileMapping.ContainsKey(partBaseInfo.mod.ID)) {
				MwcModApi.modSaveFileMapping.Add(partBaseInfo.mod.ID, partBaseInfo.saveFilePath);
			}

			if (MwcModApi.modsParts.TryGetValue(partBaseInfo.mod.ID, out var modParts)) {
				modParts.Add(id, this);
			} else {
				MwcModApi.modsParts.Add(
					partBaseInfo.mod.ID, new Dictionary<string, Part>
					{
						{ id, this }
					}
				);
		...
		public void AddScrew(Screw screw, GameObject parent)
		{
			...
			MwcModApi.screws.Add(screw.gameObject.name, screw);
		}
